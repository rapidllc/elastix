...
Main( const int argc, const char* const* const argv )
{
  ...
  const int result = [parameterFileList, outFolder, argMap]
  {
    ...
    return elastixObject.RegisterImages(fixedImage, movingImage, parameterMaps, outFolder, performLogging, performCout, fixedMask, movingMask);
  }();
...
main( int argc, char ** argv )
{
  ...
  {
    std::cerr 
      << "An error occurred! Exception: " << stdException.what() << '\n';
    return EXIT_FAILURE;
  }
}
